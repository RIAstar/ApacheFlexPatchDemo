...
public class GridItemEditor extends Group implements IGridItemEditor
{
    ...
    public function set data(value:Object):void
    {
        ...
        {
            this.value = getProperty(_data, column.dataField);
        }
    }
    ...
    public function save():Boolean
    {
        ...
        var typeInfo:String = "";
        var parentPropertyLength:int = property.lastIndexOf('.');
        if (parentPropertyLength < 0) parentPropertyLength = property.length;
        var parentProperty:String = property.substr(0, parentPropertyLength);
        var parentData:Object = getProperty(data, parentProperty);
        for each(var variable:XML in describeType(parentData).variable)
        {
        ...
        var oldData:Object = getProperty(data, property);

        if (property && oldData !== newData)
        {
            ...
            setProperty(data, property, newData);
    ...
    public static function getProperty(source:Object, property:String):* {
        var target:Object = source;
        var chain:Array = property.split('.');

        for each (var prop:String in chain) target = target[prop];

        return target;
    }

    public static function setProperty(source:Object, property:String, value:*):void {
        var target:Object = source;
        var chain:Array = property.split('.');
        var lastProp:String = chain.pop();
